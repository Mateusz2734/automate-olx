...
def login_email(driver):
    ...
    your_account = WebDriverWait(driver, timeout=10).until(
        ec.element_to_be_clickable((By.ID, 'topLoginLink')))
    your_account.click()

    email = WebDriverWait(driver, timeout=10).until(
    ...
    login_me = WebDriverWait(driver, timeout=10).until(
        ec.presence_of_element_located((By.ID, 'se_userLogin')))
    login_me.click()

    try:
        dont_show = WebDriverWait(driver, timeout=10).until(
